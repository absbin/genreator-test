...
def spanFixDim(V0, V, spanCons, FUZZ=rl_config._FUZZ):
    ...
    for (x0, x1), v in spanCons.iteritems():
        t = sum([V[x] + M.get(x, 0) for x in list(range(x0, x1 + 1))])
        if t >= v - FUZZ:
            continue  # already good enough
        X = [x for x in list(range(x0, x1 + 1)) if V0[x]
             is None]  # variable candidates
        if not X:
...
class SpreadsheetTable(Flowable):
    def __init__(self, data, colWidths=None, rowHeights=None, style=None,
        ...
        if not (nrows and ncols):
            if emptyTableAction == 'error':
                raise ValueError(
                    "%s must have at least a row and column" % self.identity())
                ...
                Preformatted.__init__(self, '%s(%d,%d)' % (
                    self.__class__.__name__, nrows, ncols), _emptyTableStyle)
        ...
        for i, d in enumerate(data):
            ...
            if n != ncols:
                if rl_config.allowShortTableRows and isinstance(d, list):
                    ...
                    raise ValueError("%s expected %d not %d columns in row %d!" % (
                        self.identity(), ncols, n, i))
    ...
    def identity(self, maxLen=30):
        ...
        if cv and 'unknown' not in (nr, nc):
            b = 0
            for i in list(range(nr)):
                for j in list(range(nc)):
                    v = cv[i][j]
    ...
    def _calc_height(self, availHeight, availWidth, H=None):
                    ...
                    if isinstance(v, (tuple, list, Flowable)):
                        ...
                        if ji in colSpanCells:
                            if not span:
                                continue
                            w = max(
                                colpositions[span[2] + 1] - colpositions[span[0]], w)
        ...
        activeRows0 = self._activeRows[0] if self._activeRows[
            0] is not None else self.repeatRows  # ugly hack to make it backward compatible
        height = self._height = sum(H[:self.repeatRows] +
                                    H[activeRows0:hmax] +
                                    H[self._nrows - self._repeatRowsB:])
        # index 0 is actually topline; we skip when processing cells
    ...
    def _hasVariWidthElements(self, upToRow=None):
        ...
        if upToRow is None:
            upToRow = self._nrows
        for row in list(range(min(self._nrows, upToRow))):
            for col in list(range(self._ncols)):
    ...
    def _calcPreliminaryWidths(self, availWidth):
        ...
        if verbose:
            print
        'prelim width calculation.  %d columns, %d undefined width, %0.2f units remain' % (
        ...
        for colNo in list(range(self._ncols)):
            w = W[colNo]
            if w is None or w == '*' or _endswith(w, '%'):
                siz = 1
                current = final = 0
                for rowNo in list(range(self._nrows)):
        ...
        if verbose:
            print
        'predefined width:   ', given
        if verbose:
            print
        'uncomputable width: ', unsizeable
        if verbose:
            print
        'computable width:   ', sizeable
    ...
    def _calcSpanRanges(self):
        ...
        for x in list(range(self._ncols)):
            for y in list(range(self._nrows)):
    ...
    def _calcSpanRects(self):
        ...
        be reused after split.
        """
        if getattr(self, '_spanRects', None):
            return
        colpositions = self._colpositions
    ...
    def _addCommand(self, cmd):
        if cmd[0] in ('BACKGROUND', 'ROWBACKGROUNDS', 'COLBACKGROUNDS'):
            ...
            sc, ec, sr, er = self._normalizeCoord(sc, ec, sr, er)
            for i in list(range(sr, er + 1)):
                for j in list(range(sc, ec + 1)):
    ...
    def _drawInnerGrid(self, sc_sr, ec_er, weight, color, count, space):
        ...
        self._drawVLines((sc + 1, sr), (ec, er + 1),
                         weight, color, count, space)
    ...
    def _drawLineBelow(self, sc_sr, ec_er, weight, color, count, space):
        ...
        # Generates line for each visible row.
        for vis in visible:
            self._drawHLines((sc, vis + 1), (ec, vis + 1),
                             weight, color, count, space)
    ...
    def _drawHLines(self, sc_sr, ec_er, weight, color, count, space):
        ...
        visible = [i for i in list(range(sr, er + 1))
                   if self._is_visible_line(i)]
    ...
    def _drawVLines(self, sc_sr, ec_er, weight, color, count, space):
        ...
        def canvLine(y0, x0, y1, x1, _line=self.canv.line): return _line(
            x0, y0, x1, y1)
        if count == 1:
            for x in cp:
    ...
    def _drawLineAfter(self, sc_sr, ec_er, weight, color, count, space):
        ...
        # Some parts visible - rendering.
        self._drawVLines((sc + 1, sr), (ec + 1, er + 1),
                         weight, color, count, space)
            ...
            if r != None:
                ...
                if y1 != y2:
                    ...
                    while 1:
                        if y >= ymax:
                            break
                        # split at position y is impossible because of overlapping rowspan
                        impossible[y] = None
                        y += 1
    ...
    def draw(self):
        ...
        if not self._spanCmds:
            ...
            for rowNo, row, rowstyle, rowpos, rowheight in zip(row_nums,
                                                               cellvalues, cellstyles, self._rowpositions[1:],
                                                               rowheights):

                for colNo, cellval, cellstyle, colpos, colwidth in zip(
                        list(range(self._ncols)), row, rowstyle,
    ...
    def _drawBkgrnd(self):
        ...
        for cmd, (sc, sr), (ec, er), arg in self._bkgrndcmds:
            ...
            visible = []
            for row_num in list(range(sr, er + 1)):
    ...
    def _drawCell(self, cellval, cellstyle, colpos_rowpos, colwidth_rowheight):
        ...
        if isinstance(cellval, (tuple, list, Flowable)):
            ...
            if valign == 'TOP':
                ...
                y = rowpos + (rowheight + cellstyle.bottomPadding -
                              cellstyle.topPadding + h) / 2.0
            if cellval:
            ...
            for v, w, h in zip(cellval, W, H):
                if just == 'LEFT':
                    ...
                    x = colpos + (colwidth + cellstyle.leftPadding -
                                  cellstyle.rightPadding - w) / 2.0
                else:
            ...
            if valign == 'BOTTOM':
                ...
                y = rowpos + (cellstyle.bottomPadding + rowheight -
                              cellstyle.topPadding + n * leading) / 2.0 - fontsize
            else:
        ...
        if cellstyle.href:
            ...
            self.canv.linkURL(cellstyle.href, (colpos, rowpos,
                                               colpos + colwidth, rowpos + rowheight), relative=1)
